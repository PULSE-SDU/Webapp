...
export class DashboardComponent {
  ...
  get deadStatus(): number {
    return this.equipmentData.filter((item) => item.status === BatteryStatus.OFFLINE).length;
  }
  ...
  get infoCards(): InfoCard[] {
    return [
      ...
      {
        title: BatteryStatus.OFFLINE,
        content: 3,
        subtitle: 'Requires immediate attention',
        icon: 'M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z',
        iconColor: StatusColor[BatteryStatus.OFFLINE],
      },
      {
        title: BatteryStatus.LOW,
        content: 2,
        ...
        iconColor: StatusColor[BatteryStatus.LOW],
      },
      {
  ...
  get equipmentNeedingAttention(): Tag[] {
    return this.equipmentData
      .filter((item) => item.status === BatteryStatus.OFFLINE || item.status === BatteryStatus.LOW)
      .sort((a, b) => {
